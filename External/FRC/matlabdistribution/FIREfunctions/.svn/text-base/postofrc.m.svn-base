...
if isempty(positions)
    if isempty(sz)
        ...
        frc_out = zeros(1,ceil(sz/sqrt(2)));
        return
    end
end
...
if size(positions,2) == 2
    ...
    blocksel = randperm(nblocks);                                      
    blocksel = blocksel(1:ceil(nblocks/2));                             % Block numbers in the first half data set
    ...
    blocksel = randperm(nblocks);                                       % Shuffle block numbers
    blocksel = blocksel(1:ceil(nblocks/2));                             % Block numbers in the first half data set
    s = ismember(positions(:,3),blocksel);                              % Localizations in the first half data set
end

% Bin images into 2 half data images
in1 = binlocalizations(positions(s,1:2),sz,sz,zm);
in2 = binlocalizations(positions(~s,1:2),sz,sz,zm);
