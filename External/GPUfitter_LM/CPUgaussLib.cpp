...
float kernel_IntGauss1D(const int ii, const float x, const float sigma) {
    ...
    //return 1.0f/2.0f*(boost::math::erf((ii-x+0.5f)*sqrt(norm))-boost::math::erf((ii-x-0.5f)*sqrt(norm)));
...
//**********************************************************************************************************************
 ...
 void kernel_DerivativeGauss2D_sigmaxy(int ii, int jj, float PSFSigma,float* theta, float *dudt, float *model) {
	float PSFx, PSFy;
	PSFx=kernel_IntGauss1D(ii, theta[0], theta[4]);
