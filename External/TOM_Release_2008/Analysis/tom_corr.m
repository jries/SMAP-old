...
if (nargin==2)
    ccf = real(ifftshift(tom_ifourier(tom_fourier(b).*conj(tom_fourier(a)))))/(size(a,1)*size(a,2)*size(a,3));%perform correlationend;
end;
...
if (nargin == 4)
    ...
    ccf = real(ifftshift(tom_ifourier(tom_fourier(b).*conj(tom_fourier(a)))))./n;%perform correlation
end;
