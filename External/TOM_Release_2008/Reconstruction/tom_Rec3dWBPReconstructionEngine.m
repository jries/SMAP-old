...
for k=1:NumOfProj
    ...
    % Isoscale projection
    if isequal(isoscaleResult(k), 1) ~= 1
         
         % Scale projection
         ProjScale = imresize(Proj, isoscaleResult(k));
         clear Proj;
         
         % ProjScale bigger than Proj
         if isoscaleResult(k) > 1
              
              % Cut ProjScale
              Proj(1:ImdimResult, 1:ImdimResult) = ...
                   ProjScale(floor(size(ProjScale,1)./2)-ImdimResult/2+1:floor(size(ProjScale,1)./2)+ImdimResult/2, ...
                                   floor(size(ProjScale,2)./2)-ImdimResult/2+1:floor(size(ProjScale,2)./2)+ImdimResult/2);
         
         % ProjScale smaller than Proj
         elseif isoscaleResult(k) < 1
                  
              % Zero frame
              Proj = zeros(ImdimResult, ImdimResult, 'double');
              
              % Even ProjScale size
              if isequal(mod(size(ProjScale,1),2),0) == 1
                   
                   % Paste ProjScale
                   Proj(ImdimResult/2-floor(size(ProjScale,1)./2)+1:ImdimResult/2+floor(size(ProjScale,1)./2), ...
                           ImdimResult/2-floor(size(ProjScale,1)./2)+1:ImdimResult/2+floor(size(ProjScale,1)./2)) ...
                   = ProjScale(1:size(ProjScale,1), 1:size(ProjScale,2));
              
              % Odd ProjScale size
              elseif  isequal(mod(size(ProjScale,1),2),1) == 1
                   
                   % Paste ProjScale
                   Proj(ImdimResult/2-floor(size(ProjScale,1)./2):ImdimResult/2+floor(size(ProjScale,1)./2), ...
                          ImdimResult/2-floor(size(ProjScale,1)./2):ImdimResult/2+floor(size(ProjScale,1)./2)) ...
                   = ProjScale(1:size(ProjScale,1), 1:size(ProjScale,2));
         ...
         % ProtocolMode
         if strcmp(ProtocolMode, 'on') == 1
              disp(['                         Isoscaled with factor: ' num2str(isoscaleResult(k))]);
         end
    ...
    if strcmp(ApplyWeighting, 'on') == 1
         ...
         if strcmp(WeightingMethod, 'exact') == 1
              WeightingFunction = tom_calc_weight_function ...
              ([ImdimResult ImdimResult], [ProjDirResult; Tiltangles]', ...
              ObjectThickness, [ProjDirResult(k) Tiltangles(k)]);
         end
