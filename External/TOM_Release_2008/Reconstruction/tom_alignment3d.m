function [Matrixmark, psi, sigma, x, y, z]  = tom_alignment3d(Matrixmark, irefmark, imintilt, r, imdim)
...
if (length(y) < length(x))%%%%%%%%%%%%%%%%%%%% updated by Thomas Haller, 08.08.2007
    y((length(y)+1):length(x)) = 1000000;    % Fix error, for last points undefined
end;
if (length(z) < length(x))
    z((length(z)+1):length(x)) = 1000000;    
end;%%%%%%%%%%%%%%%%%%%%%%%%%
