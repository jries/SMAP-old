...
/* 2D Rotation */
void rot2d (float *image,float *rotimg,long sx,long sy,float *p_phi,char ip,float px,float py,int euler_dim)
{
...
{
const int *dims_i, *dims_o,*dimension_euler; /*dimension */
int dimension;
char *ip;
...
if (dimension==3) 
{
	...
	rot3d(mxGetData(INP),mxGetData(OUT),dims_i[0],dims_i[1],dims_i[2],mxGetData(EULER),ip[0],center[0],center[1],center[2],dimension_euler[0]);
}
