% \file GaussMLEv2_sigmaxy.m
% \author Peter Relich
% \date June 02, 2011

function[P, CRLB, LL] = GaussMLEv2_sigmaxy(datastack, PSFSigma, iterations)
    ...
    maxjump = [1e0, 1e0, 1e2, 2e0, 1e-1, 1e-1];
    gamma = [1.0, 1.0, 0.5, 1.0, 1.0, 1.0];
    ...
    for kk = 1:iterations % Main iterative loop
        ...
        for ii = 1:sz
            for jj = 1:sz
                ...
                PSFy = IntGauss1D( jj-1, theta(2,:), theta(6,:) );
                
                model = theta(4,:) + theta(3,:).*PSFx.*PSFy;
                data = permute(datastack(ii,jj,:),[1,3,2]);
                
                % Calculating Derivatives
                [dudt(1,:),d2udt2(1,:)] = DerivativeIntGauss1D( ii-1, theta(1,:), theta(5,:), theta(3,:), PSFy );
                [dudt(2,:),d2udt2(2,:)] = DerivativeIntGauss1D( jj-1, theta(2,:), theta(6,:), theta(3,:), PSFx );
                
                [dudt(5,:),d2udt2(5,:)] = DerivativeIntGauss1DSigma( ii-1, theta(1,:), theta(5,:), theta(3,:), PSFy );
                [dudt(6,:),d2udt2(6,:)] = DerivativeIntGauss1DSigma( jj-1, theta(2,:), theta(6,:), theta(3,:), PSFx );
        ...
        theta(6,:) = max( theta(6,:), PSFSigma/10.0 );
    end
    ...
    for ii = 1:sz
        for jj = 1:sz
            ...
            % Calculating Derivatives
            dudt(1,:) = DerivativeIntGauss1D( ii-1, theta(1,:), theta(5,:), theta(3,:), PSFy );
            dudt(2,:) = DerivativeIntGauss1D( jj-1, theta(2,:), theta(6,:), theta(3,:), PSFx );
            dudt(5,:) = DerivativeIntGauss1DSigma( ii-1, theta(1,:), theta(5,:), theta(3,:), PSFy );
            dudt(6,:) = DerivativeIntGauss1DSigma( jj-1, theta(2,:), theta(6,:), theta(3,:), PSFx );
            dudt(3,:) = PSFx.*PSFy;
            dudt(4,:) = ones(1,Nfits);
