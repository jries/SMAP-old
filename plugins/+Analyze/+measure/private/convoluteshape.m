function [fitout,fout,fstart]=convoluteshape(xv,profile,sigma,structure)
...
switch structure
    ...
    case 1 %stepfunction
        start=[m,s1,s2-s1,0]
        fitout=lsqcurvefit(@fitstep,double(start),double(xv),double((profile)),[],[],newopts,sigma);
        fout=fitstep(fitout,xv,sigma);
        fstart=fitstep(start,xv,sigma);
        
   case 2 %disk

%         a0=fitdisk([1,x0s,(s2-s1)/2,0],x0s,sigma);
        start=[m,x0s,(s2-s1)/2,mean(profile(1:3))];
        fitout=lsqcurvefit(@fitdisk,double(start),double(xv),double((profile)),[0 0 0 0],[inf inf inf inf],newopts,sigma);
        
        fout=fitdisk(fitout,xv,sigma);  
        fstart=fitdisk(start,xv,sigma);
%           fout=fitdisk(start,xv,sigma); 
   case 3 %ring
        start=[m,x0s,(s2-s1)/2,mean(profile(1:3))];
        fitout=lsqcurvefit(@fitring,double(start),double(xv),double((profile)),[],[],newopts,sigma);
        fout=fitring(fitout,xv,sigma);
         fstart=fitring(start,xv,sigma);
   case 4 %two structure: distance. Gaussian Approximation
       start=[m,m,s1,s2-s1,0.3*(s2-s1),0];
        fitout=lsqcurvefit(@fit2gauss,double(start),double(xv),double((profile)),[],[],newopts,sigma);
        fout=fit2gauss(fitout,xv,sigma);
         fstart=fit2gauss(start,xv,sigma);

end

function out=fit2gauss(p,xv,sigma)
%p=[A,x0,R], 
A1=p(1);
A2=p(2);
x0=p(3);
...
function out=fitstep(p,xv,sigma)
%p=[A,x0,L], 
A=p(1);
x0=p(2);
L=p(3);
...
R=p(3);
off=p(4);
out=quadv(@(x)convdisk(x,xv,sigma,R,x0),-R+x0,R+x0);
