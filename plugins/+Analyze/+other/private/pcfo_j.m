...
for ty=0:TilesY-1
    for tx=0:TilesX-1
        xStart = floor((tx*size(in,1))/TilesX);
        xEnd   = floor(((tx+1)*size(in,1))/TilesX)-1;
        yStart = floor((ty*size(in,2))/TilesY);
        yEnd   = floor(((ty+1)*size(in,2))/TilesY)-1;       
        myTile = in(xStart+1:xEnd+1,yStart+1:yEnd+1,frame);
        myTile=squeeze(myTile);
        if (prod(size(myTile)) > 0) %just in case, should not happen
            NumPixelsInBin(n) = prod(size(myTile));
            [TotalVar(n), TotalInt(n)] = noisevariance_j(myTile, kthres, AvoidCross); % this does not need any information on offset or readout noise
            n=n+1;
        end
    end
end
...
for k=sim(3):-1:1
    [AllTotalVarh(k), AllTotalInth(k)] = noisevariance_j(in(:,:,k), kthres, AvoidCross);
%     AllTotalVar2
end
