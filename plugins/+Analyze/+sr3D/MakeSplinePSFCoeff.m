classdef MakeSplinePSFCoeff<interfaces.DialogProcessor
    ...
    methods
        function obj=MakeSplinePSFCoeff(varargin)   %replace by filename        
        ...
        function out=run(obj,p)
            ...
            %example:
            oldf=obj.getPar('loc_fileinfo');
            if isempty(oldf)
                deffile='*.tif';
            else
                deffile=[fileparts(oldf.basefile) filesep '*.tif'];
            end
...
%             out.error='this error occured because you did something wrong'; %if an error occoured, you can output it in the status bar with this command.
        ...
        function pard=guidef(obj)
           %pard structure can be =[]; All fields are optional.
            ...
            pard.text1.object=struct('String','dz','Style','text');
