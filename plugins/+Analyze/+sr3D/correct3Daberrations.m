...
%              obj.guiPar.FieldHeight=obj.guiPar.FieldHeight-1;obj.guiPar.Vrim=obj.guiPar.Vrim-20;
        ...
        function out=run(obj,p)
            ...
            p.axhere=[];
            %get interpolation
           [ZcorrInterp]=getZinterp(beads,{},p);
            %calculate errors
           ...
           while 1
                ...
                [ZcorrInterp]=getZinterp(beads2,dzerr2,p);
                %calculate errors
           ...
           [ZcorrInterp]=getZinterp(beads2,dzerrh,p);
...
function [Zint]=getZinterp(beads,dzerr,p)
 %make big array for interpolation
 zglassall=[];z0relativeall=[];zfitall=[];idall=[];zplot=[];dzerrall=[]
            for k=1:length(beads)
...
%             zplot=z0relativeall-zfitall;
            ...
            xrange=min(zglassall(inz)):200:max(zglassall(inz));
