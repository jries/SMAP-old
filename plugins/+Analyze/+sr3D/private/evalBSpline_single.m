function bi = evalBSpline_single(xi,deg)
...
switch deg
    ...
    case 2
        ...
        x2 = xi.*xi;
        k = (xi>=1/2) & (xi<3/2);
        bi(k) = single(9/8) - single(3/2).*xi(k) + single(1/2).*x2(k);
        k = (xi>=-1/2) & (xi<1/2);
        bi(k) = single(3/4) - x2(k);
        k = (xi>=-3/2) & (xi<-1/2);
        bi(k) = single(9/8) + single(3/2).*xi(k) + single(1/2).*x2(k);
    case 3
        bi = zeros(size(xi));
        x2 = single(xi.*xi); x3 = single(x2.*xi); 
        k = (xi>=1) & (xi<2);
        %test by YL
%         k = (xi>=1) & (xi<=2);
        bi(k) = single(4/3) - 2.*xi(k) + x2(k) - single(1/6).*x3(k);
        k(:) = (xi>=0) & (xi<1);
        bi(k) = single(2/3) - x2(k) + single(1/2).*x3(k);
        k(:) = (xi>=-1) & (xi<0);
        bi(k) = single(2/3) - x2(k) - single(1/2).*x3(k);
        k(:) = (xi>=-2) & (xi<-1);
        bi(k) = single(4/3) + 2.*xi(k) + x2(k) + single(1/6).*x3(k);
