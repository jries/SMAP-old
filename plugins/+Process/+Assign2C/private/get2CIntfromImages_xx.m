...
% compare intensities
    ...
    [maxposA,maxposAR]=nm2pixLoc(xA,yA,pix_cam,filestruc.info.roi);
    roiA=roiCutter.run(imageframe,maxposAR);
    bgA=roiCutter.run(bg,maxposAR);
    
    [maxposB,maxposBR]=nm2pixLoc(xB,yB,pix_cam,filestruc.info.roi);
...
%     locfitsort=assignside(locfit,{'fit_n','fit_bg'},x(ind1:ind2),y(ind1:ind2),transform.refPart,midp);
    ...
    locout=copyfields(locsum,locfit);

%     for k=1:length(fieldsout)
%         locout.(fieldsout{k})(indref)=locall.(fieldsout{k});
%         locout.(fieldsout{k})(indtarget)=locall.(fieldsout{k});
%     end
end
