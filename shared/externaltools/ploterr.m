...
catch error %('x and y are not consistent in size.')
end
...
try [xl,xh]=expandarrays(xerr{1},xerr{2}); catch error
    %('xl and xh are not consistent in size.'), 
end
try [y,xl,ty,txl]=expandarrays(y,xl); catch error%('xl and y are not consistent in size.'), 
end
if ty, y=y'; xl=xl'; txl=~txl; end % make sure x and y still match
if txl, xh=xh'; end % make sure xl and xh still match
try [y,xh]=expandarrays(y,xh); catch error%('xh and y are not consistent in size.'), 
end
if relxerr
    try [xl,x,txl]=expandarrays(xl,x); catch error%('xl and x are not consistent in size.'), 
    end
    if txl, xh=xh'; end
    try [x,xh]=expandarrays(x,xh); catch error%('xh and x are not consistent in size.'), 
    end
    xl=x+xl; xh=x+xh;
end
...
try [yl,yh]=expandarrays(yerr{1},yerr{2}); catch error%('yl and yh are not consistent in size.'), 
end
try [x,yl,tx,tyl]=expandarrays(x,yl); catch error%('yl and x are not consistent in size.'), 
end
if tx, x=x'; yl=yl'; tyl=~tyl; end % make sure x and y still match
if tyl, yh=yh'; end % make sure yl and yh still match
try [x,yh]=expandarrays(x,yh); catch error%('yh and x are not consistent in size.'), 
end
if relyerr
    try [y,yl]=expandarrays(y,yl); catch error%('yl and y are not consistent in size.'), 
    end
    try [y,yh]=expandarrays(y,yh); catch error%('yh and y are not consistent in size.'), 
    end
    yl=y+yl; yh=y+yh;
end
