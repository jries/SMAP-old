function [P,update, error, model_save] =  kernel_MLEfit_Spline_LM_multichannel(d_data1,d_data2,coords1chip, coords2chip, tform, coeff1,coeff2,sz,iterations)
   ...
   ystart2 = floor(yc2);
    ...
    for ii = 0:sz-1
        for jj = 0:sz-1
            ...
            [newDudt1, model1] =  kernel_DerivativeSpline_v2(ii+xstart1+off,jj+ystart1+off,zstart,spline_xsize,spline_ysize,spline_zsize,delta_f1,delta_dxf1,delta_dyf1,delta_dzf1,coeff1,newTheta1,1);
...
%             end
            ...
            if data1>0
                newErr1 = newErr1 +2*((model1-data1)-data1*log(model1/data1));
            else
                newErr1 = newErr1 + 2*model1;
                data1 = 0;
            end
            
            if data2>0
                newErr2 = newErr2 +2*((model2-data2)-data2*log(model2/data2));
            else
                newErr2 = newErr2 + 2*model2;
                data2 = 0;
            end
