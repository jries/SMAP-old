function [P,update, error, model_save] =  kernel_MLEfit_Spline_LM_multichannel1(d_data1,d_data2,coords1chip, coords2chip, tform, coeff1,coeff2,sz,iterations)
...
for tx = 1:Nfits
    ...
    [Nmax1,newTheta(5)] = kernel_GaussFMaxMin2D(sz,PSFSigma,single(d_data1(sz*sz*(tx-1)+1:sz*sz*(tx))));  
%     [Nmax2,newTheta(7)] = kernel_GaussFMaxMin2D(sz,PSFSigma,single(d_data2(sz*sz*(tx-1)+1:sz*sz*(tx))));
%     newTheta(7) = newTheta(6);
    newTheta(4)=max(0,(Nmax1-newTheta(5))*2*pi*PSFSigma*PSFSigma);
...
%     newTheta(6) = 0;
    ...
    maxJump = [1, 1,spline_zsize/5,100,100];
