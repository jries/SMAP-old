...
void  NMS(double *maxima,float *img,short sx,short sy,short n, long maxmax)
{
...
for(i1=n;i1<sy-n-n;i1+=n+1) //carful if sx and sy are exchanged 
{
    for(j1=n;j1<sx-n-n;j1+=n+1) //extra n subtracted so indices cannot run out. Maybe rim is lost.
    {
        mi=i1; mj=j1;
...
//         end
        ...
        for(i2=mi-n;i2<=mi+n;i2+=1) //+1???
        {
            ...
            {
               if ((i2<i1|i2>i1+n)|(j2<j1|j2>j1+n))
               {
                    if(*(img+i2*sx+j2) > *(img+mi*sx+mj)) 
                    {
                        stop=1;
