...
for(k=0;k<numlocs;k++)
    ...
    if(xr>=-dnx&&xr<srec[0]+dnx&&yr>=-dny&&yr<srec[1]+dny)
        ...
        dx=xpix[k]-xr;
        dy=ypix[k]-yr;
        intcorrectionx=(erf((dnx+0.5)/sigmax[0]/1.4142135624));
        intcorrectiony=(erf((dny+0.5)/sigmay[0]/1.4142135624));
        //intcorrection=1;
        gaussnorm=N[k]/(2*3.1415926536*sigmax[0]*sigmay[0]*intcorrectionx*intcorrectiony);
        if(uselut==1)
            {
                    ...
                    if(uselut==1)
                        ...
                        if(indc>=0&indc<sl)
                            ...
                            for(col=0;col<3;col++)
                                ...
                                srim[srimindlin]=oldfac*inttemp*lut[indc+col*sl]+(1-oldfac)*srim[srimindlin];
                                }
                            }
                        }
                     else
                        ...
                        srim[srimindlin]=oldfac*inttemp+(1-oldfac)*srim[srimindlin];
                        }
...
{
   float *srim;
  float *Gtemplate,*xpix,*ypix,*sigmax,*sigmay,*N,*c,*transparency;
  float *lut,*rangec;
  float Gsigma,roiks;
  int uselut;
  double numberOfLocs;
  ...
  /* xpix, ypix, srec, sigma, 4.Gtemplate, 5. Gsigma, roiks, 7. N, uselut, 9. c, 10. lut, 11. rangec,transparency matrix*/ 
 ...
 transparency=(float*)mxGetData(prhs[13]);
