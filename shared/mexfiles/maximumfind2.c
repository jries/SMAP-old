...
void  NMS(double *maxima,float *img,short sx,short sy, long maxmax)
{
...
for(i1=1;i1<sy-1;i1+=1) //carful if sx and sy are exchanged 
{
    for(j1=1;j1<sx-1;j1+=1) //extra n subtracted so indices cannot run out. Maybe rim is lost.
    {
       /* stop=0;
        for(i2=i1-1;i2<=i1+1;i2+=1)
        {
            for(j2=j1-1;j2<=j1+1;j2+=1)
            {
                if((*(img+i2*sx+j2) >= *(img+i1*sx+j1)) &~( i1==i2 & j1==j2))
                {
                    stop=1;
                    break;
                }
                ...
                (*(img+i1*sx+j1) > *(img+(i1+1)*sx+j1+1)))               
        {
            ...
            *(maxima+maxind)=i1+1;
//             maxind++;
            *(maxima+maxind+maxmax)=j1+1;
            *(maxima+maxind+2*maxmax)=*(img+i1*sx+j1);
            maxind++;
            j1++; //no two maxima can be side by side
        }
    }
//     *(maxima)=5;
}
