...
%global variables
if isempty(pos.x)
    if nargin<6||length(lut)==1||isempty(lut)||isempty(rangec)
        sz=3;
    else
        sz=1;
    end
    nx=round((rangex(2)-rangex(1))/pixelsx);
    ny=round((rangey(2)-rangey(1))/pixelsy);
    srim=zeros(nx,ny,sz);
    nlocs=0;
    return
end
...
xspix=pos.sx/pixelsx;
% xspixmed=median(xspix(~isnan(xspix)));
% xspixmed=min(myquantilefast(xspix(~isnan(xspix)),.5,1000)*10,200);
% xspix(xspix>xspixmed)=xspixmed;

yspix=pos.sy/pixelsy;
% yspixmed=median(yspix(~isnan(yspix)));
% yspix(yspix>xspixmed)=xspixmed;
...
[srim,nlocs]=gaussrender_Tx_elliptc(single(xpix-1),single(ypix-1),uint32(srec),single(xspix),single(yspix),single(G.shifted),single(G.sigmatemplate),...
    single(roiks*.6),single(N),int32(uselut),single(pos.c),single(lut), single(rangec),single(G.ellipse));
...
shift=sigmatemplate/2;
gausstemplate.shifted=exp(-((Xg+shift).^2+(Yg+shift).^2)/2/sigmatemplate^2);
