function [Yl,S] = dtwavexfm2_L(X,nlevels,w1,w2)
...
if nlevels >= 1,
   ...
   Lo = colfilter(X,w1.h0o).';
%    Hi = colfilter(X,h1o).';
   
   % Do odd top-level filters on rows.
   LoLo = colfilter(Lo,w1.h0o).';			% LoLo
   S{1}=size(LoLo);
%    Yh{1} = zeros([size(LoLo)/2  6]);
%    Yh{1}(:,:,[1 6]) = q2c(colfilter(Hi,w1.h0o).');			% Horizontal pair
...
if nlevels >= 2;
   for level = 2:nlevels;
      ...
      Lo = coldfilt(LoLo,w2.h0b,w2.h0a).';
%       Hi = coldfilt(LoLo,h1b,h1a).';
      
      % Do even Qshift filters on columns.
      LoLo = coldfilt(Lo,w2.h0b,w2.h0a).';	%LoLo
      S{level}=size(LoLo);
%       Yh{level} = zeros([size(LoLo)/2  6]);
%       Yh{level}(:,:,[1 6]) = q2c(coldfilt(Hi,w2.h0b,w2.h0a).');	% Horizontal
